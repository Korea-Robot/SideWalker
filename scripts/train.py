...
class CNNFeatureExtractor(nn.Module):
    ...
    def __init__(self, input_channels=6, feature_dim=512):  # # RGB(3) + Depth(3)  or Semantic(3) + Depth(3)
        super().__init__()
...
class PPOConfig:
    ...
    entropy_coef: float = 0.01  # entropy ??
    max_grad_norm: float = 0.5  # grad maximum 제한
    ppo_epochs: int = 4         
    batch_size: int = 1024
    buffer_size: int = 4096     # buffer size가 batch에 비해 큰 이유 : 
    ...
    try:
        ...
        while True:
            ...
            experience = {
                ...
                'log_prob': torch.tensor(0.0)  # 임시값 (실제로는 정책에서 계산)
            }
...
def main():
    ...
    # 여러 프로세스(즉, 메인 프로세스와 워커 프로세스) 간에 데이터를 안전하게 주고받을 수 있도록 해주는 큐
    ...
    for i in range(NUM_WORKERS):
        worker = mp.Process(
            target=env_worker,
            args=(i, data_queue, True),  # True = training mode
            name=f"EnvWorker-{i}"
        )
        worker.start()
        workers.append(worker)
    
    print(f"Started {NUM_WORKERS} environment workers")
    ...
    try:
        while total_steps < 100000:  # 총 스텝 수 제한
            # 워커로부터 경험 데이터 수집
            try:
                experience = data_queue.get(timeout=10.0)  # 10초 타임아웃
                agent.add_experience(experience)
                total_steps += 1
                
                # 배치 크기만큼 데이터가 모이면 학습
                if len(agent.buffer) >= config.batch_size:
                    print(f"\nUpdating model... (Step: {total_steps})")
                    agent.update()
                    update_count += 1
                    
                    if update_count % 10 == 0:
                        print(f"Progress: {total_steps}/100000 steps, {update_count} updates")
                
            except:
                print("Timeout waiting for data from workers")
                continue
