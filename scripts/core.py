...
"""
core upgrade 사항 : git issue에도 발행함

1. wandb logging. lossfunction에 각각 요소와 avg reward 등 전부 wandb log로 넣어줘.

2. 지금 목표지점이 너무 가까워서 바로 terminated 되는데 그걸 long horizon으로계속 맞추게 하고 싶어.
3. 지금 policy function은 policy network에서 normal D를 가져오는데 아래처럼 initial Mixture D Problem 이미 어떤게 좋은 방향인지 대략 알고 있다. 왜냐하면 goal_position, goal_direction이 존재하니까. 따라서 무작정 start action에 대해 학습 시키는것보다 prior D를 주는게 효율적일까? Solution 따라서 initial action D에 대해 3가지 mixture D를 사용할 예정 1. Policy 분포(현재 네트워크) 2. Goal direction을 mean으로 하는 prior normal D 3. Exploration을 위한 random(uniform D) 학습이 진행될수록 prior D의 가중치는 줄이고, exploration은 고정, policy는 가중치 증가
replay buffer 여러번 ppo에서 잘 사용하기.
Problem

4. 경험 버퍼(replay buffer) 내 데이터의 분포가 mdp에 따라 iid는 아니지만 데이터간 correlation이 줄어서 variance가 줄어들어 gradient를 더낮은 분산으로 학습시킬수있음.
Solution
Data Efficiency : 같은 데이터를 여러 번 반복적으로 학습
하나의 buffer로 여러번 minibatch를 뽑아서 학습
일반 PPO/actor-critic 학습에서는 전체를 tensor로 묶어서 여러 번 사용하는 게 표준적
