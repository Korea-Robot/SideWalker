...
class SharedFeatureExtractor(nn.Module):
    ...
    def forward(self, rgb: torch.Tensor, depth: torch.Tensor, goal: torch.Tensor) -> torch.Tensor:
        ...
        rgb_features = rgb_features.view(batch_size, -1)  # [batch, 1280]
        
        # Depth Feature Extraction  
        depth_features = self.depth_encoder(depth)
        depth_features = self.depth_pool(depth_features)
        depth_features = depth_features.view(batch_size, -1)  # [batch, 512]
        
        # Feature Fusion
        # EfficientNet-B0: 1280, PerceptNet: 512, Goal: 2
        fused_features = torch.cat([rgb_features, depth_features, goal], dim=1)  # [batch, 1794]
        
        return fused_features

# --- 네트워크 정의 (MODIFIED to use shared features) ---
class Actor(nn.Module):
    def __init__(self, shared_feature_extractor: SharedFeatureExtractor, hidden_dim=512, output_dim=2):
        super().__init__()
        
        self.shared_feature_extractor = shared_feature_extractor
        
        # Feature fusion dimension: 1280 (EfficientNet) + 512 (PerceptNet) + 2 (goal) = 1794
        fusion_dim = 1794
    ...
    def forward(self, rgb: torch.Tensor, depth: torch.Tensor, goal: torch.Tensor) -> torch.distributions.MultivariateNormal:
        ...
        fused_features = self.shared_feature_extractor(rgb, depth, goal)
...
class Critic(nn.Module):
    def __init__(self, shared_feature_extractor: SharedFeatureExtractor, hidden_dim=512):
        super().__init__()
...
# --- 메인 학습 코드 (MODIFIED to use shared features) ---
def main():
    # WandB 초기화
    wandb.init(
        project="metaurban-rl-shared-features",
        config={
            "train_epochs": 200,
    ...
    # 공유 특징 추출기 초기화
    shared_feature_extractor = SharedFeatureExtractor().to(device)
    
    # 네트워크 초기화 (공유 특징 추출기 사용)
    actor = Actor(shared_feature_extractor, hidden_dim=config.hidden_dim).to(device)
    critic = Critic(shared_feature_extractor, hidden_dim=config.hidden_dim).to(device)
    
    # 옵티마이저 (공유 파라미터 포함)
    # 공유 특징 추출기의 파라미터는 두 옵티마이저 모두에서 업데이트됩니다
    actor_params = list(actor.parameters())  # 이미 shared_feature_extractor 파라미터 포함
    critic_params = list(critic.parameters())  # 이미 shared_feature_extractor 파라미터 포함
    
    actor_optimizer = torch.optim.Adam(actor_params, lr=config.actor_lr)
    ...
    # 모델 저장
    torch.save(actor.state_dict(), 'metaurban_actor_shared_features.pt')
    torch.save(critic.state_dict(), 'metaurban_critic_shared_features.pt')
    torch.save(shared_feature_extractor.state_dict(), 'metaurban_shared_features.pt')
