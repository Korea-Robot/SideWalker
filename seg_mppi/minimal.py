...
def build_occupancy_from_tdsm_colors(tdsm_bgr, tol=10, k_close=5, k_open=3, inflation_radius=8):
    """
...
def create_enhanced_visualization(tdsm_bgr, occ, free_mask, path, start_rc, goal_rc, local_rc):
    ...
    if path is not None and len(path) >= 2:
        ...
        # 메인 흰색 선 (더 굵게)
        cv2.polylines(vis, [pts], False, (255, 255, 255), 8, cv2.LINE_AA)
...
def process_semantic_map_and_plan(tdsm_bgr, ego_goal_position, inflation_radius=8, 
...
parser.add_argument("--inflation_radius", type=int, default=28, help="Obstacle inflation radius in pixels")
