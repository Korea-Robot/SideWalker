...
# --- [최적화 1] 실시간성을 위한 입력 리사이즈 ---
# True로 설정하면, ROS 이미지를 이 크기로 먼저 리사이즈한 후 추론합니다.
# 고해상도(4K 등) 카메라 사용 시 속도 향상에 매우 효과적입니다.
...
MODEL_TYPE_INDEX = 3  # <--- 🚀 여기를 변경하여 모델 선택 (예: 3 (coco))
    ...
    def __init__(self, pretrained_model_name="nvidia/mit-b0", num_classes=7):
        ...
        try:
            self.original_model = SegformerForSemanticSegmentation.from_pretrained(
                ...
                # [최적화] torch_dtype=torch.float32 제거 -> autocast가 작동하도록 허용
...
class SegformerViewerNode(Node):
    def __init__(self):
        ...
        # --- [최적화 2] CUDNN 벤치마크 활성화 ---
        if self.device.type == 'cuda':
            torch.backends.cudnn.benchmark = True
            self.get_logger().info("CUDNN 벤치마크 모드 활성화됨.")
        # ------------------------------------
    ...
    def load_model_and_processor(self):
        """
        [개선된 함수]
        MODEL_TYPE에 따라 적절한 모델, 프로세서, 클래스 맵을 로드하고,
        공통 최적화(torch.compile)를 적용합니다.
        """
        ...
        if self.model_type in ['surface', 'object']:
            ...
            # model_name = "facebook/maskformer-swin-base-coco" # <--- 느림
            model_name = "facebook/maskformer-swin-tiny-coco" # <--- 빠름 (기존과 동일)
        ...
        # --- [최적화 3] torch.compile 적용 (Pytorch 2.0+ 이상) ---
        self.get_logger().info("모델 컴파일 시도 (torch.compile)...")
        try:
            if hasattr(torch, 'compile'):
                # 'reduce-overhead' 모드는 컴파일 시간을 줄이고 추론 속도를 높여 실시간에 적합
                model = torch.compile(model, mode="reduce-overhead")
                self.get_logger().info("모델 컴파일 성공!")
            else:
                self.get_logger().warn("torch.compile을 사용할 수 없습니다 (PyTorch 2.0+ 필요).")
        except Exception as e:
            self.get_logger().warn(f"torch.compile 실패: {e}. 컴파일 없이 계속합니다.")
        # ----------------------------------------------------
    ...
    def image_callback(self, msg):
        ...
        try:
            img_bgr_full = self.bridge.imgmsg_to_cv2(msg, "bgr8")
        except Exception as e:
            self.get_logger().error(f"ROS 이미지 변환 실패: {e}")
            return
            
        # --- [최적화 1] 입력 이미지 사전 리사이즈 ---
        if ENABLE_PRE_RESIZE:
            img_bgr = cv2.resize(img_bgr_full, (PRE_RESIZE_W, PRE_RESIZE_H), interpolation=cv2.INTER_LINEAR)
        else:
        ...
        with torch.no_grad():
            ...
            # --- [최적화 4] FP16(Half Precision) 추론을 위한 autocast ---
            use_autocast = self.device.type == 'cuda'
            
            # autocast 컨텍스트 내에서 모델 추론만 수행
            with torch.autocast(device_type="cuda", dtype=torch.float16, enabled=use_autocast):
                if self.model_type == 'ade20k':
                    outputs = self.model(**inputs) # outputs = SegformerOutput
                elif self.model_type == 'coco':
                    outputs = self.model(**inputs) # outputs = MaskFormerOutput
                else: # 'surface', 'object'
                    outputs = self.model(inputs) # outputs = 텐서 (logits)
            # --------------------------------------------------------

            # --- 후처리 (autocast 바깥에서 수행) ---
            ...
            if self.model_type in ['surface', 'object', 'ade20k']:
                ...
                # 업샘플링 (원본 이미지 크기로 복원)
                # [최적화] .float()로 캐스팅하여 FP16/BF16 -> FP32로 변경 후 보간 (수치 안정성)
                ...
                # 1. [중요] Panoptic 후처리 (프로세서 사용, autocast 밖에서)
                result = self.processor.post_process_panoptic_segmentation(
                    outputs, target_sizes=[(original_h, original_w)]
                )[0] # 0번 = 첫 번째(유일한) 이미지 결과
                
                # (H, W) 크기의 텐서. 각 픽셀은 '인스턴스 ID'를 가짐 (CPU 텐서)
                pred_mask_np = result["segmentation"].cpu().numpy()
                # 인스턴스 ID별 정보 리스트
                segment_info = result["segments_info"]

                # --- [최적화 5] MaskFormer 시각화: NumPy LUT(Look-Up Table) 사용 ---
                # 기존 루프 방식(segmentation_image[pred_mask_np == instance_id] = color)은
                # 객체가 많을수록 매우 느려집니다.
                
                # 1. 최대 인스턴스 ID를 찾아 LUT 크기 결정
                max_instance_id = 0
                if segment_info: # 감지된 객체가 있을 경우
                    max_instance_id = max(info['id'] for info in segment_info)
                
                # 2. LUT 생성 (0번 ID는 배경=검은색으로 초기화)
                lut = np.zeros((max_instance_id + 1, 3), dtype=np.uint8)
                
                # 3. LUT 채우기 (인스턴스 ID -> 클래스 색상)
                for info in segment_info:
                    instance_id = info['id']
                    class_id = info['label_id'] # 0=unlabeled, 1=person ...
                    # 팔레트에서 해당 '클래스'의 색상 조회
                    color = self.color_palette[class_id % self.NUM_LABELS]
                    lut[instance_id] = color
                
                # 4. NumPy의 'fancy indexing'을 사용하여 마스크 전체에 색상 동시 적용
                segmentation_image = lut[pred_mask_np]
                # -------------------------------------------------------------
        ...
        # 화면에 표시
        cv2.imshow("Multi-Model Segmentation Viewer (Optimized)", images_to_show)
