...
class ReconstructionNode(Node):
    ...
    def __init__(self, config: ReconstructionConfig = None):
        ...
        self.get_logger().info(f'📡 Subscribing to: {self.config.rgb_topic}, {self.config.depth_topic}')
        self.get_logger().info(f'📤 Publishing to: {self.config.pointcloud_topic}')
    ...
    def _setup_subscribers(self):
        ...
        qos = QoSProfile(
            reliability=ReliabilityPolicy.BEST_EFFORT,  # RealSense는 BEST_EFFORT 사용
        ...
        self.get_logger().info('✅ Synchronized RGB-D subscribers created')
    ...
    def _setup_publishers(self):
        ...
        self.get_logger().info('✅ Point cloud publisher created')
    
    def _setup_tf(self):
        """Setup TF2 transform listener"""
        self.tf_buffer = Buffer()
        self.tf_listener = TransformListener(self.tf_buffer, self)
        self.get_logger().info('✅ TF2 listener created')
    ...
    def rgbd_callback(self, depth_msg, rgb_msg):
        ...
        try:
            ...
            try:
                ...
                # TF 실패시에도 source frame으로 publish
                pass
            self.timings['transform'].append((time.perf_counter() - t4) * 1000)
            
            # 6. Downsample and filter invalid points
            points, colors, labels = self._downsample(points, colors, labels)
            points, colors, labels = self._filter_valid_points(points, colors, labels)
            
            # 7. Publish
            if points.shape[0] > 0:  # 유효한 포인트가 있을 때만 publish
                pc_msg = self._create_pointcloud_msg(points, colors, labels, depth_msg.header)
    ...
    def _filter_valid_points(self, points, colors, labels):
        """Filter out invalid points (NaN, Inf, zero depth)"""
        # Remove invalid points
        valid_mask = np.isfinite(points).all(axis=1) & (points[:, 2] > 0.1)  # z > 0.1m
    ...
    def _create_pointcloud_msg(self, points, colors, labels, original_header):
        """Create PointCloud2 message"""
        header = Header()
        header.stamp = original_header.stamp  # 원본 타임스탬프 사용
        ...
        return PointCloud2(
            ...
            data=data.tobytes(),
            is_dense=False,  # NaN 필터링 후이지만 False가 더 안전
        )
    ...
    def _print_stats(self):
        ...
        fps = len(self.timings['total']) / 5.0  # 5초마다 리포트
        msg.extend([
            f"{'='*60}",
