...
def setup_visualization(node):
    ...
    # ★★★ 변경된 부분 (1) ★★★
    # 최적 궤적: zorder를 높여 항상 위에 보이게 함
    optimal_traj_line, = ax.plot([], [], 'm-', lw=3.0, zorder=12,
    ...
    for i in range(node.num_samples_to_plot):
        ...
        # ★★★ 변경된 부분 (2) ★★★
        # 샘플 궤적: alpha(0.2->0.4), lw(0.5->0.8) 상향 조정
        line, = ax.plot([], [], 'c-', lw=1.8, alpha=0.5, zorder=5, label=label)
