...
def setup_visualization(node):
    ...
    # (수정) wps_array는 (N, 3) [x, y, yaw] 형태임.
    # x, y 좌표만 슬라이싱하여 min/max를 계산해야 함.
    if wps_array.size == 0:
         # 웨이포인트가 없는 경우 기본값 설정
         x_min, y_min = -5.0, -5.0
         x_max, y_max = 5.0, 5.0
    else:
         # [:, :2] -> 모든 행의, 0번 1번 열 (x, y)만 선택
         x_min, y_min = wps_array[:, :2].min(axis=0) - 1.5
         x_max, y_max = wps_array[:, :2].max(axis=0) + 1.5
