...
class MPPIController:
    ...
    def run_mppi(self, local_goal_tensor, costmap_tensor):
        ...
        weights = torch.exp(-1.0 * costs_normalized / self.lambda_ )
        weights /= (torch.sum(weights) + 1e-9) # (K,)

        # 6. 가중 평균을 사용하여 평균 제어 시퀀스(U) 업데이트
        # 가중 평균된 노이즈 weights : K, noise : K,T,2 => [T,2] 
        weighted_noise = torch.einsum('k,ktu->tu', weights, noise)
        self.U = self.U + weighted_noise
        # TODO : why..? self.U prior가 너무 강력하게 작용하는거아니야? self.U를 기본으로하고 weighted_noise가 어떻게 동작하는거야?
        # iterate many times. then it is move good way
