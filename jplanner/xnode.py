...
"""
semantic_bev_node.py

메인 ROS 2 노드.
- Depth, RGB 이미지를 동기화하여 수신
- 'semantic_model'을 사용하여 시맨틱 세그멘테이션 수행
- GPU 가속을 통해 Semantic Point Cloud와 Semantic BEV Map을 발행
"""
...
from xmodel import SemanticModel
# --------------------------------
...
class SemanticPointCloudBEVNode(Node):
    ...
    def __init__(self):
        ...
        # 'reconstruction_config.py'에서 ReconstructionConfig 클래스를 로드
        ...
        # 'semantic_model.py'에서 SemanticModel 클래스를 로드
        ...
        self.declare_parameter('semantic_pointcloud_topic', self.config.pointcloud_topic)
        ...
        self.semantic_pointcloud_fields = [
            ...
            PointField(name='label', offset=16, datatype=PointField.UINT32, count=1), # uint32 사용 (float32와 4바이트)
        ]
        self.point_step_pcl = 20 # 4*4 (float32) + 4 (uint32) = 20 bytes

        # 5.2. Semantic BEV Map 필드 (x,y,z,rgb,label)
        # PCL과 동일한 20바이트 필드를 사용 (Rviz 호환성)
    ...
    def process_and_publish_semantic_pointcloud(
        ...
        # (float32로 비트캐스팅하는 이유는 .tobytes() 효율을 위해)
        ...
        pointcloud_msg = self._create_semantic_cloud_from_data(
            data_np, stamp, self.target_frame, is_bev=False
        )
    ...
    def process_and_publish_semantic_bev(
        ...
        # bev_ignore_labels_t 텐서를 직접 사용 (CPU-GPU 동기화 방지)
        for label in self.bev_ignore_labels_t: 
        ...
        self.bev_packed_flat.fill_(0) # 이전 프레임 데이터 초기화
        self.bev_packed_flat.index_reduce_(
            dim=0,
            index=linear_indices,
            source=packed_data,
            reduce="amax", # 최대값 (가장 높은 Z)
            include_self=False
        )
        ...
        bev_msg = self._create_semantic_cloud_from_data(
            bev_data_np, stamp, self.target_frame, is_bev=True
        )
    ...
    def _create_semantic_cloud_from_data(self, data_np, stamp, frame_id, is_bev=False):
        """
        ...
        # PCL과 BEV 모두 동일한 20바이트 (x,y,z,rgb,label) 필드 사용
