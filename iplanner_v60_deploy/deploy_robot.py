...
class RealSensePlannerControl(Node):
    def __init__(self):
        ...
        self.goal = torch.tensor([2.0, 0.0, 0.0], dtype=torch.float32)  # 2m 전방 목표
        self.current_cmd = Twist()
        
        # angular gain
        self.angular_gain = 20
    ...
    def control_callback(self):
        ...
        try:
            with torch.no_grad():
                ...
                # 로그
                self.get_logger().info(f"Cmd: linear_x={linear_x:.3f}, angular_z={angular_z:.3f}, fear={fear.item():.3f}")
