...
def update_plot(i, node, traj_line, start_point, current_point, heading_line, ax):
    ...
    # 궤적 라인 데이터를 업데이트합니다.
    # 기존: traj_line.set_data(data[:, 0], data[:, 1])
    # 변경: x와 y 좌표를 서로 바꿈
    traj_line.set_data(data[:, 1], data[:, 0])

    # 시작점과 현재점 데이터를 업데이트합니다.
    # 기존: start_point.set_data(data[0, 0], data[0, 1])
    # 변경: x와 y 좌표를 서로 바꿈
    start_point.set_data(data[0, 1], data[0, 0])
    
    # 기존: current_point.set_data(data[-1, 0], data[-1, 1])
    # 변경: x와 y 좌표를 서로 바꿈
    ...
    # 헤딩 라인 데이터를 업데이트합니다.
    # 기존: heading_line.set_data([current_x, end_x], [current_y, end_y])
    # 변경: x와 y 좌표를 서로 바꿈
    ...
    # 레전드 업데이트
    # 기존: current_label = f'Current Pos ({current_x:.2f}, {current_y:.2f})'
    # 변경: 레이블의 순서를 바꿈
...
def main(args=None):
    ...
    # 플롯 설정
    ax.set_title('Real-time Robot Trajectory with Swapped Axes')
    # 기존: ax.set_xlabel('X Position (m)') / ax.set_ylabel('Y Position (m)')
    # 변경: 축 레이블을 서로 바꿈
